...
func (b *Bot) buildAssistantSystemPrompt(ctx context.Context) (staticPart, dynamicPart string) {
	...
	staticPart = "Ты — универсальный личный ассистент владельца WhatsApp-бота учёта финансов. Тебя зовут " + b.botName + " — " +
		...
		"10g. missing_in_group — какие чеки за период попали в ДРУГИЕ группы, но НЕ попали в указанную ('какие чеки не " +
		"попали в Оплата КЛНТ, но есть в других', 'что не добавлено в Оплата КЛНТ'). Совпадение чека — по номеру операции, " +
		"а без него по клиенту+сумме. Целевую группу бери из запроса.\n" +
